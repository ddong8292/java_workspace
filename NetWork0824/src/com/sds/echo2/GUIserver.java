...
public class GUIserver extends JFrame implements ActionListener, Runnable{
	...
	JTextArea area;
	JScrollPane scroll;
	ServerSocket server;//접속 감지용 소켓!! 대화용 X
	Thread acceptTread;//접속사 감지용 쓰레드
	BufferedReader buffr;
	BufferedWriter buffw;
	
	//ArrayList와 기능은 동일하나, 동시성 문제에 있어서는 동기화를 지원해주므로, 쓰레드에 안전하다.
	...
	public void startServer(){
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "포트번호는 숫자로 넣어주세요");
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
		}
