 ...
 * 				장점 : 객체화시켜 제어할 수 있어서 객체지향 언어방식으로 
 * 						프로그램이 가능
 * 				단점 : 메모리사용량이 많으므로 특히, 모바일기기는 불리하다.
 * 2.SAX - 실행부가 각 태그를 발견할때마다 적절한 이벤트가 발생하는 방식이고, 
 * 				개발자는 이 이벤트 타이밍을 잡아내서 원하는 처리를 해줘야하는 방식.
...
public class ParserTest {
	...
	//SAXparser가 추상클래스이므로 직접 new 할 수 없고, 아래의
	//SAXparserFactory 객체로부터 인스턴스를 얻어와야 한다.
